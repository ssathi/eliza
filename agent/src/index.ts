...
import { HealthcoachClientInterface } from "@elizaos/client-healthcoach";
...
import { healthcoachPlugin } from "@elizaos/plugin-healthcoach";
...
export async function initializeClients(
    ...
    if (clientTypes.includes(Clients.HEALTHCOACH)) {
        const healthcoachClient = await HealthcoachClientInterface.start(runtime);
        if (healthcoachClient) {
            clients.healthcoach = healthcoachClient;
        }
    }
...
export async function createAgent(
    ...
    return new AgentRuntime({
        ...
        plugins: [healthcoachPlugin].filter(Boolean),
        providers: [],
        managers: [],
        cacheManager: cache,
        fetch: logFetch,
